...
# Note: date_to_month is used for pivot_months_loops and date_to_month_pandas() is for
#       pivot_months_pandas().
def date_to_month_pandas(d):
    # A modified version of date_to_month()
    def year_it(y):
        return '%04i' % y
    year = d.dt.year.apply(year_it)

    def month_it(m):
        return '%02i' % m
    month = d.dt.month.apply(month_it)

    returnme = year.str.cat(month, sep='-')
...
def pivot_months_pandas(data):
    ...
    """
    # ...
    # 1. add a column 'month' that contains results of applying date_to_mon on existing
    #    'date' column

    data['month'] = date_to_month_pandas(data['date'])
    ...
    aggregatedSum = grouped_data.aggregate('sum').reset_index()
    aggregatedCount = grouped_data.aggregate('count').reset_index()

    # 3. Use the Pandas pivot method to create a row for each station(name) and colummns
    #    for each month. Repeat with 'count' aggregation

    monthly = aggregatedSum.pivot(index='name', columns='month')['precipitation']
    counts = aggregatedCount.pivot(index='name', columns='month')['precipitation']
