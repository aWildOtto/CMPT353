...
# Function to use pipeline to change RGB to LAB
def x_transform_LAB(x_arr):
	x_new = x_arr.reshape(1,-1,3)
	x_new = rgb2lab(x_new)
	x_new = x_new.reshape(-1,3)
	return x_new

def x_transform_HSV(x_arr):
	x_new = x_arr.reshape(1,-1,3)
	x_new = rgb2lab(x_new)
	x_new = x_new.reshape(-1,3)
	return x_new
...
def main():
    ...
    bayes_convert_model = make_pipeline(FunctionTransformer(x_transform_HSV), GaussianNB() )

    knn_rgb_model = KNeighborsClassifier(n_neighbors=10)
    knn_convert_model = make_pipeline(FunctionTransformer(x_transform_HSV), KNeighborsClassifier(n_neighbors=10) )

    rf_rgb_model = RandomForestClassifier(n_estimators=400, max_depth=10, min_samples_leaf=10)
    rf_convert_model = make_pipeline(FunctionTransformer(x_transform_HSV), RandomForestClassifier(n_estimators=400, max_depth=10, min_samples_leaf=10) )
